...
def pre_evaluate_process(plans: list) -> list:
    if not plans or not isinstance(plans[0], list):
        plans = [plans]  # 将非嵌套列表转为嵌套列表
...
class TreeBuilder:
    ...
    def __featurize_others(self, node, depth):
        arr = np.zeros(len(ALL_TYPES))
        return np.concatenate((arr, self.__stats(node), [depth]))

    def __featurize_join(self, node, depth):
        assert is_join(node)
        arr = np.zeros(len(ALL_TYPES))
        arr[ALL_TYPES.index(node["Node Type"])] = 1
        return np.concatenate((arr, self.__stats(node), [depth]))  # [hint one-hot || buf, card., row, depth]

    def __featurize_scan(self, node, depth):
        assert is_scan(node)
        arr = np.zeros(len(ALL_TYPES))
        arr[ALL_TYPES.index(node["Node Type"])] = 1
        return (np.concatenate((arr, self.__stats(node), [depth])),
                self.__relation_name(node))  # [hint one-hot || buf, card., row, depth || relation name ]

    def plan_to_feature_tree(self, plan, depth=0):
        children = plan["Plans"] if "Plans" in plan else []
        if len(children) == 1:
            return self.plan_to_feature_tree(children[0], depth + 1)

        if is_join(plan):
            # assert len(children) == 2
            my_vec = self.__featurize_join(plan, depth)
            left = self.plan_to_feature_tree(children[0], depth + 1)
            right = self.plan_to_feature_tree(children[1], depth + 1)
            return my_vec, left, right

        if not is_scan(plan) and not is_join(plan):
            assert len(children) == 2
            my_vec = self.__featurize_others(plan, depth)
            left = self.plan_to_feature_tree(children[0], depth + 1)
            right = self.plan_to_feature_tree(children[1], depth + 1)
            return my_vec, left, right
        ...
        if is_scan(plan):
            assert not children
            return self.__featurize_scan(plan, depth)
