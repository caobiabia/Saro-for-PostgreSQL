...
PG_CONNECTION_STR_JOB = "dbname=imdbload user=postgres password=postgres port=5432"
PG_CONNECTION_STR_TPCH = "dbname=tpchload user=postgres password=postgres port=5432"
...
# print([i for i in all_48_hint_sets])
...
def run_query(sql):
    ...
    try:
        cur.execute("SET statement_timeout TO 300")  # 3000s 50min 最长
        cur.execute(sql)
        cur.fetchall()
        conn.close()
    except:
...
def run_query_hint(sql, hints):
    ...
    # execute SQL, if reach the statement_timeout, then the exe time = statement_timeout + 1
    try:
        cur.execute("SET statement_timeout TO 600000")  # 3000s 50min 最长
        cur.execute(sql)
        cur.fetchall()
        conn.close()
    except:
